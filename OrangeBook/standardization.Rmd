...
replaceData(originalData = wheatData, newData = newTradeData,
            mergeKey = params$mergeKey)
## Manually reduce the bulgur imports to create a deficit.  Makes
## standardization more interesting later.
...
toPrint = wheatData[geographicAreaM49 == currentCountry & timePointYears == currentYear, ]
toPrint[, updateFlag := (element == "5141" & measuredItemCPC == "0111")]
printTable(toPrint, params)
...
Note that in this case, the standard deviation for food is very small; this is because it's mostly determined by the production of wheat flour, and this value is an official figure.

**ADD BALANCING HERE**
```{r, echo = FALSE}
invisible({
    wheatData[measuredItemCPC == "0111" & timePointYears == "2011" &
                  element == "5141", Value := 26278543]
    wheatData[, c(params$adjustVar) := NA_real_]
    wheatData[measuredItemCPC == "0111" & timePointYears == "2011" &
                  element == "5141", c(params$adjustVar) := 26278543 - 26283777]
})
```
...
foodDelta = wheatData[measuredItemCPC == "0111" & timePointYears == "2011" &
                  element == "5141", get(params$adjustVar)]
...
wheatData = rollDownFoodDelta(wheatData)
```

...
- Implement calculation for tourist element.
- Implement calculation for industrial element.
- Implement calculation for feed element.
- Implement calculation for residual element.
...
# NATA:
params = defaultStandardizationParameters_Nata()
...
fbsElements = c(params$productionCode, params$feedCode, params$seedCode,
                params$wasteCode, params$foodCode, params$stockCode,
                params$importCode, params$exportCode, params$foodProcCode, 
                params$touristCode,params$industrialCode,params$feedCode)
...
touristVar = paste0(params$elementPrefix, params$touristCode)
fullData[, c(touristVar) := NA_real_]
industrialVar = paste0(params$elementPrefix, params$industrialCode)
fullData[, c(industrialVar) := NA_real_]
feedVar = paste0(params$elementPrefix, params$feedCode)
fullData[, c(feedVar) := NA_real_]
...
# NATA
pulledElements = c("5113", "5025", "5312", "5510", "5421", "5520", "5525",
                   "5023", "5327", "5016", "5141", "5120", "f???","tou","ind")
...
tree = rbind(tree, data.table(childID = "23520", target = "B",
                              extractionRate = 0.92, parentID = c(23511.01, 23512),
...
industrialEstimates[, standardDeviation_measuredElement_ind :=
                   Value_measuredElement_ind * 0.1]
replaceData(wheatData,
            industrialEstimates[, c("Value_measuredElement_ind",
                               "standardDeviation_measuredElement_ind",
                               params$mergeKey),
                           with = FALSE],
            mergeKey = params$mergeKey)
wheatData[element == "ind" & Value == 0, Value := NA]
toPrint = wheatData[geographicAreaM49 == currentCountry &
                        timePointYears == currentYear, ]
toPrint[, updateFlag := element == "ind"]
printTable(toPrint, params, workingDir)
warning("Standard error for tourist data is currently just 10% of industrial value, it ",
        "is not estimated in any way.")
```
...
load(paste0(workingDir, "/touristEstimates_Nata.RData"))
touristEstimates[, standardDeviation_measuredElement_tou :=
                   abs(Value_measuredElement_tou) * 0.1]
###### HELP!!!
replaceData(wheatData,
            touristEstimates[, c("Value_measuredElement_tou",
                               "standardDeviation_measuredElement_tou",
                               params$mergeKey),
                           with = FALSE],
            mergeKey = params$mergeKey)
wheatData[element == "tou" & Value == 0, Value := NA]
toPrint = wheatData[geographicAreaM49 == currentCountry &
                        timePointYears == currentYear, ]
toPrint[, updateFlag := element == "tou"]
printTable(toPrint, params, workingDir)
warning("Standard error for tourist data is currently just 10% of tourist value, it ",
        "is not estimated in any way.")
```
...
cattleData = data.table:::melt.data.table(data = cattleData,
    id.vars = c(params$geoVar, params$itemVar, params$yearVar))
cattleData[grepl("Value", variable), variableType := "Value"]
cattleData[grepl("flagObservationStatus", variable), variableType := "obsFlag"]
cattleData[grepl("flagMethod", variable), variableType := "metFlag"]
castFormula = paste(paste(params$mergeKey, collapse = " + "), "~ variableType")
# temp = tidyr::spread(data = cattleData, formula = as.formula(castFormula),
#                             value.var = "value")
cattleData[, variable := gsub("[A-Za-z_]*_", "", variable)]
setnames(cattleData, "variable", "element")
cattleData = tidyr::spread(data = cattleData, key = "variableType", value = "value")
## Remove alot of the data, and "impute" it later
cattleData[, Value := rep(0, nrow(cattleData))]
...
toPrint = cattleData[geographicAreaM49 == currentCountry & timePointYears == currentYear &
                        measuredItemCPC %in% c("02111", "21111"), ]
## Force cattle to be missing except for area harvested
# toPrint[measuredItemCPC == "0111" & element %in% c("5510", "5421"), Value := 0]
# toPrint[measuredItemCPC == "0111" & element == "5312", Value := 
#     fullData[measuredItemCPC == "0111" & timePointYears == currentYear &
#                  geographicAreaM49 == currentCountry, Value_measuredElement_5312]]
...
In this case, the production value is only known for cattle flour (it is missing for cattle), and for cattle we are also missing the yield value.  The first step in the imputation process is to impute the yield, using the previously described production imputation methodology.
...
```{r, echo = FALSE, eval = FALSE}
suppressWarnings({suppressMessages({invisible({
cattleData = merge(cattleData,
                  fullData[measuredItemCPC %in% cattleKeys,
                           c("Value_measuredElement_5510", params$mergeKey),
                           with = FALSE], by = params$mergeKey)
cattleData[element == "5510", Value := Value_measuredElement_5510]
cattleData[, standardDeviation := 0]
cattleData[element == "5510" & Value == 0, Value := NA]
cattleData[, Value_measuredElement_5510 := NULL]
})})})
```
...
```{r, echo = FALSE, eval = FALSE}
suppressWarnings({suppressMessages({
newTradeData = fullData[measuredItemCPC %in% cattleKeys,
                        c("Value_measuredElement_5600", params$mergeKey),
                        with = FALSE]
newTradeData[, standardDeviation_measuredElement_5600 := 0]
replaceData(originalData = cattleData, newData = newTradeData,
            mergeKey = params$mergeKey)
newTradeData = fullData[measuredItemCPC %in% cattleKeys,
                        c("Value_measuredElement_5900", params$mergeKey),
                        with = FALSE]
...
## Manually tweak trade to create a deficit.  Makes standardization more
## interesting later.
cattleData[element == "5600" & measuredItemCPC == "23140.02" &
              timePointYears == "2011", Value := 182485]
cattleData[element == "5900" & measuredItemCPC == "23140.03" &
              timePointYears == "2011", Value := 217289]
cattleData[element == "5600" & Value == 0, Value := NA]
cattleData[element == "5900" & Value == 0, Value := NA]
...
setnames(stockEstimates, c("Value", "sdEstimate"),
         paste0(c("Value", "standardDeviation"), "_measuredElement_71"))
replaceData(cattleData,
            stockEstimates[, c("Value_measuredElement_71",
                               "standardDeviation_measuredElement_71",
                               params$mergeKey),
                           with = FALSE],
            mergeKey = params$mergeKey)
cattleData[element == "71" & Value == 0, Value := NA]
toPrint = cattleData[geographicAreaM49 == currentCountry & timePointYears == currentYear, ]
toPrint[, updateFlag := element == "71"]
printTable(toPrint, params, workingDir)
})})
...
The allocation to food, on the other hand, can potentially be considered at any processing level, although some commodities (such as cattle) are assumed to not be eaten as such.  We impute food consumption numbers for the example country and update the FBS table below.
...
replaceData(cattleData,
            ...
            mergeKey = params$mergeKey)
# If no food is estimated yet, it doesn't mean that it should be forced to be 0.
# cattleData[element == "5141" & Value == 0, Value := NA]
toPrint = cattleData[geographicAreaM49 == currentCountry &
                        timePointYears == currentYear, ]
toPrint[, updateFlag := element == "5141" & Value > 0]
printTable(toPrint, params, workingDir)
})})
...
replaceData(cattleData,
            lossEstimates[, c("Value_measuredElement_5120",
                           ...
                           with = FALSE],
            mergeKey = params$mergeKey)
cattleData[element == "5120" & Value == 0, Value := NA]
toPrint = cattleData[geographicAreaM49 == currentCountry &
                        timePointYears == currentYear, ]
toPrint[, updateFlag := element == "5120"]
printTable(toPrint, params, workingDir)
...
Feed allocation must be done at this phase in order to ensure that we have reduced the feed demand by the corresponding amounts of feed products.
...
replaceData(cattleData,
            seedEstimates[, c("Value_measuredElement_5525",
                           ...
                           with = FALSE],
            mergeKey = params$mergeKey)
cattleData[element == "5525" & Value == 0, Value := NA]
toPrint = cattleData[geographicAreaM49 == currentCountry &
                        timePointYears == currentYear, ]
toPrint[, updateFlag := element == "5525"]
printTable(toPrint, params, workingDir)
...
```{r, echo = FALSE, eval = FALSE}}
load(paste0(workingDir, "/industrialEstimates.RData"))
industrialEstimates[, Value_measuredElement_ind :=
...
replaceData(cattleData,
            touristEstimates[, c("Value_measuredElement_tou",
                           ...
                           with = FALSE],
            mergeKey = params$mergeKey)
cattleData[element == "tou" & Value == 0, Value := NA]
toPrint = cattleData[geographicAreaM49 == currentCountry &
                        timePointYears == currentYear, ]
toPrint[, updateFlag := element == "tou"]
printTable(toPrint, params, workingDir)
...
```{r, echo = FALSE, fig.height=12, fig.width = 18, eval = FALSE}
suppressWarnings({suppressMessages({
edges = tree[parentID %in% cattleKeys, ]
setnames(itemName, "Item", params$parentVar)
edges = merge(edges, itemName, by = params$parentVar, all.x = TRUE)
edges[, c("parentID", "Name") := list(Name, NULL)]
setnames(itemName, params$parentVar, params$childVar)
...
```{r, echo = FALSE, eval = FALSE}
# save(cattleData, file = paste0(workingDir, "preStandardizedcattle.RData"))
# load(paste0(workingDir, "preStandardizedcattle.RData"))
suppressWarnings({suppressMessages({
toPrint = cattleData[geographicAreaM49 == currentCountry & timePointYears == currentYear, ]
printTable(toPrint, params, workingDir)
})})
...
## Production = Exports + Food - Imports
## 5510 = 5900 + 71 - 5600
computeProcessedProduction(data = cattleData, tree = tree, standParams = params)
cattleData[element == "5510" & Value < 0, Value := 0]
toPrint = cattleData[geographicAreaM49 == currentCountry & timePointYears == currentYear, ]
toPrint[, updateFlag := (element == "5510" & Value < 18000000)]
printTable(toPrint, params, workingDir)
})})
...
distributionTable = toPrint[measuredItemCPC %in% c(flourCode, bulgurCode,
                            ...
                            list(measuredItemCPC, Value, standardDeviation)]
setnames(distributionTable, c("Item", "prodMean", "prodSd"))
distributionTable[, cattleMean := prodMean / c(.72, .95, 1, .22)]
distributionTable[, cattleSd := prodSd / c(.72, .95, 1, .22)]
printStandardizationTable(distributionTable, params, workingDir = workingDir)
})})
```
...
```{r, echo = FALSE, eval = FALSE}
suppressWarnings({suppressMessages({
cattleData = rollUpFood(data = cattleData, tree = tree, standParams = params)
toPrint = cattleData[geographicAreaM49 == currentCountry &
                        timePointYears == currentYear, ]
toPrint[, updateFlag := (element == params$foodProcCode &
                             measuredItemCPC == "0111")]
...
cattleData = merge(cattleData,
                  toPrint[, !c("Value", "metFlag", "obsFlag", "standardDeviation"),
                          with = FALSE],
                  by = c(params$mergeKey, "element"), all.x = TRUE)
cattleData[!is.na(balancedValue), Value := balancedValue]
cattleData[, balancedValue := NULL]
toPrint[, Value := balancedValue]
toPrint[, c("adjustment", "balancedValue") := NULL]
printDistributionTable(toPrint, params)
})})
...
```{r, echo = FALSE, eval = FALSE}
suppressWarnings({suppressMessages({
foodDelta = -cattleData[measuredItemCPC == "0111" & timePointYears == "2011" &
                  element == params$foodProcCode, get(params$adjustVar)]
## Determine adjustment by balancing: change(first element) + change(second) +
## ... = total change, so we have an equation to balance.
sink("temp.txt") # Prevent writing balancing output to document
...
sink()
distributionTable[is.na(adjustment), adjustment := 0]
distributionTable[, cattleMean := cattleMean + adjustment]
distributionTable[, prodMean := prodMean + adjustment * c(.72, .95, 1, .22)]
printStandardizationTable(distributionTable, params, workingDir = workingDir)
})})
```
...
## Allocate down the adjustments in the true/automated way
# save(cattleData, file = paste0(workingDir, "Standardizedcattle1.RData"))
# load(paste0(workingDir, "Standardizedcattle1.RData"))
sink("temp.txt") # Sink balancing output so it doesn't get printed.
cattleData = rollDownFoodDelta(data = cattleData, tree = tree,
                              standParams = params,
                              feedElements = c("23140.01", "39120.01"))
sink()
toPrint = cattleData[geographicAreaM49 == currentCountry & timePointYears == currentYear, ]
...
```{r, echo = FALSE, eval = FALSE}
suppressMessages({suppressWarnings({
finalSuaBalance(cattleData, standParams = params,
                feedElements = c(branCode, germCode))
toPrint = cattleData[timePointYears == "2011", ]
toPrint[, updateFlag := element == params$foodCode &
            measuredItemCPC %in% c(starchCode, flourCode)]
printTable(toPrint, params, workingDir)
})})
...
params$elementPrefix = ""
finalValues = finalStandardizationToPrimary(data = cattleData, tree = standTree,
                                            standParams = params)
finalValues = finalValues[timePointYears == "2011", ]
printTable(finalValues, standParams = params, workingDir)
})})
...
toPrint = cattleData[timePointYears == "2011" & element == params$foodCode, ]
toPrint = merge(toPrint, nutrientData, by = params$itemVar, all.x = TRUE)
...
```{r, echo = FALSE, eval = FALSE}
## We're done with cattle, so let's remove that data to make sure we don't
## reference it later on accident.
rm(cattleData)
```
...
```{r, echo = FALSE, eval = FALSE}
suppressWarnings({suppressMessages({
loadload(paste0(workingDir, "/touristEstimates_Nata.RData"))
touristEstimates[, standardDeviation_measuredElement_tou :=
                   abs(Value_measuredElement_tou) * 0.1]
###### HELP!!!
replaceData(sugarData,
            touristEstimates[, c("Value_measuredElement_tou",
                           ...
                           with = FALSE],
            mergeKey = params$mergeKey)
sugarData[element == "tou" & Value == 0, Value := NA]
toPrint = sugarData[geographicAreaM49 == currentCountry &
                        timePointYears == currentYear, ]
toPrint[, updateFlag := element == "tou"]
printTable(toPrint, params, workingDir)
...
```{r, echo = FALSE}
# save(sugarData, file = paste0(workingDir, "preStandardizedSugar.RData"))
...
The next step in this process is to balance the processed commodities by creating production values.  These production values will require an amount of food processing from the parent commodities, and we must start this process at the bottom of the tree, in this case considering "Sugar and Syrups nes" and "Sugar Confectionary" and going up to "Refined Sugar".

```{r, echo = FALSE}
suppressWarnings({suppressMessages({

# Manually input the production/food processing values to show the process.
toPrint[, tempProd := na2zero(.SD[element == params$exportCode, Value]) -
            na2zero(.SD[element == params$importCode, Value]) +
            na2zero(.SD[element == params$stockCode, Value]) +
            na2zero(.SD[element == params$foodCode, Value]) +
            na2zero(.SD[element == params$foodProcCode, Value]) +
            na2zero(.SD[element == params$feedCode, Value]) +
            na2zero(.SD[element == params$wasteCode, Value]) +
            na2zero(.SD[element == params$seedCode, Value]) +
            na2zero(.SD[element == params$industrialCode, Value]) +
            na2zero(.SD[element == params$touristCode, Value]) +
            na2zero(.SD[element == params$residualCode, Value]),
...
Next, we move up the tree to the balancing of refined sugar and the food processing required in the beet and cane sugar elements.  We don't have any cane sugar data, so all the production is assumed to come from beet sugar.  Note that in this case the extraction rate is not unity, thus we must divide by the extraction rate to compute the amount of food processing (in this case, 0.92):
...
invisible(toPrint[measuredItemCPC == 23520 &
            element == params$productionCode,
        c("Value", "updateFlag") := list(tempProd, TRUE)])
invisible(toPrint[measuredItemCPC == 23512 &
            element == params$foodProcCode,
        c("Value", "updateFlag") := list(
            toPrint[measuredItemCPC == 23520 &
                        element == params$productionCode, Value]/.92, TRUE)])
printTable(toPrint, params, workingDir)
})})
```

Now, we must balance all the first level processing elements.  In this example, there are only two: beet sugar and molasses.  The beet sugar case is simple: we have official production data and supply exceeds utilization; thus, we don't need to make any adjustments (other than creating food processing of sugar beet to account for the production of beet sugar).  However, the molasses case is much more challenging: we have official production that must be created from one of two parents: sugar cane or sugar beet.  Both parents have production, and thus we generate the molasses production according to availability:

```{r, echo = FALSE}
temp = data.frame(Name = c("Sugar Beet", "Sugar Cane"),
                  Availability = c(26214040+194543-304-205504,
                                   26511598+9725-861-213257),
                  Percent = c(26202775/(26202775+26307205), 26307205/(26202775+26307205)))
temp$Percent = paste0(round(temp$Percent, 3)*100, "%")
knitr::kable(temp, align = c("r", "r", "r"))
```

Thus, the amount of food allocated to processing for sugar beet and sugar cane is:

```{r, echo = FALSE}
temp = data.frame(Name = c("Sugar Beet", "Sugar Cane"),
                  beetSugar = c(4561000, 0),
                  beetSugarEq = c(4561000, 0),
                  Molasses = c(2075000*.4990056, 2075000*0.5009944),
                  molassesEq = c(2075000*.4990056, 2075000*0.5009944)/.1)
temp$Total = paste0("**", round(temp$beetSugarEq + temp$molassesEq, 0), "**")
colnames(temp) = c("Name", "Beet Sugar", "Beet Sugar Eq.", "Molasses",
                   "Molasses Eq.", "Total")
knitr::kable(temp, align = "r")
```

Thus, the updated table is:

```{r, echo = FALSE}
toPrint$updateFlag = FALSE
invisible({
toPrint[measuredItemCPC == "01801" & element == params$foodProcCode,
        c("Value", "updateFlag") := list(14915366, TRUE)]
toPrint[measuredItemCPC == "01802" & element == params$foodProcCode,
        c("Value", "updateFlag") := list(10395634, TRUE)]
})
printTable(toPrint, params, workingDir)
```
...
Now, we must balance the primary products in this table (i.e. sugar cane and sugar beet).  To do this, we need to extract the computed **standard deviations (is this the word we're using???)** of each element.  The table below shows the expected value and estimated **standard deviation** for each of the elements for wheat:

```{r, echo = FALSE}
toPrintBeet = toPrint[measuredItemCPC == "01801", ]
printDistributionTable(toPrintBeet, params)
toPrintCane = toPrint[measuredItemCPC == "01802", ]
